      ...
      <el-scrollbar style="height:calc(100% -  10px - 48px)">
        ...
        <el-form :model="form" :rules="rules" ref="dataForm" class="xt_form" label-width="110px">
          <el-row :span="6">
            ...
            <el-col :span="6">
              <el-form-item label="产权单位" prop="ownerUnit" :rules="[{ required: true, message: '产权单位不能为空'}]">
                <xt-tree-lzay-select :formValue="form.ownerUnit"
                                     :selectType="'CORP'"
            ...
            <el-col :span="6">
              <el-form-item label="产权部门" prop="propertyDept" :rules="[{ required: true, message: '产权部门不能为空'}]">
                <xt-tree-lzay-select :formValue="form.propertyDept"
                                     :unit-id="form.ownerUnitCode"
                                     :selectType="'DEPT'"
...
export default {
  components: { devOperationAddSeletDevice, devOperationDeviceEdit, xtTreeLzaySelect },
  data() {
    return {
      showLoading: false,
    ...
    initForm() {
      ...
      // 新增
        ...
        getEmpty().then(res => {
          ...
          this.form.ownerUnit = res.data.ownerUnitName 
          this.form.ownerUnitCode = res.data.ownerUnit 
          this.form.propertyDeptCode = res.data.propertyDept 
          this.form.propertyDept = res.data.propertyDeptName 
          this.$forceUpdate()
        })
        this.$forceUpdate()
      }
