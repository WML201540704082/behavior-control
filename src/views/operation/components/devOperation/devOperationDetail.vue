<template>
  <basic-container>
    ...
    <div class="page_body">
      <el-scrollbar :style="{height: showAudit? 'calc(100% -  10px - 136px - 48px)':'calc(100% -  10px - 48px)'}">
        <formTitle :titleText="'处理进度'" :titleType="'page_title'"></formTitle>
        <!-- <p @click="asd">弹框</p> -->

        <el-row type="flex">
          <el-col :span="24" style="height: 120px;padding-top:20px">
            <step
              :processKey="'bpm_sbsqckty'"
              :orderId="form.id"
              v-if="form.id"
              :currentSteps="form.processStatus"
              :orderNo="form.operationNo"
              :status="form.processStatus == 'hussar_12'"
      ...
      <!-- 底部返回按钮-->
      <div class="audit_box" v-if="showAudit">
  ...
  methods: {
    ...
    getDetail() {
      ...
      if(this.idd){
        ...
        params = { operationNo: this.$route.query.operationNo }
      }
