<template>
  <basic-container>
    <breadcrumb path="/xcnanr/authManage/temporaryAuth/index" master-title="临时授权" child-title="新增"></breadcrumb>
    <el-form class="xt_form" ref="form" :model="form" inline :rules="rules" label-suffix=":" label-width="130px" v-loading="pageLoading" style="width: 1300px;margin: 0 auto 40px auto;">
      <el-row>
        ...
        <el-col :span="12">
          <div class="grid-content bg-purple">
            <el-form-item label="授权类型" prop="mode">
              <el-select v-model="form.authType" placeholder="请选择授权类型" style="width: 500px;" @change="changeAuthTypeFunc">
                <el-option v-for="dict in dictForType" :key="dict.dictKey" :label="dict.dictValue" :value="dict.dictKey" />
              </el-select>
            </el-form-item>
          </div>
        </el-col>
    ...
    <device-dialog v-if="deviceDialogFlag" :dialog-flag="deviceDialogFlag" :auth-device-type-list="distForAuthDeviceType" @closeDialog="closeDialogFunc" @submitDialog="loadSelectedDeviceFunc" />
  </basic-container>
</template>
...
export default {
  components: { DeviceDialog, Breadcrumb },
  props: {
    childDialogFlag: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    // 关闭新增临时授权tab
    closeTab() {
      this.$router.push({ path: '/xcnanr/authManage/temporaryAuth/index' })
