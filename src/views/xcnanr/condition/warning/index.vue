  ...
  <basic-container>
    ...
    <div class="page_form" style="padding-bottom: 15px">
      <el-form ref="searchForm" :model="searchForm" label-width="80px" class="xt_search_form">
        <el-row>
          ...
          <!-- 告警来源 -->
          <el-col :span="6">
            <el-form-item label="告警来源" props="source">
              <el-select v-model="searchForm.source" filterable clearable placeholder="请选择告警来源">
                <el-option v-for="dict in sourceOptions" :key="dict.dictKey" :label="dict.dictValue" :value="dict.dictKey" />
              </el-select>
            </el-form-item>
          </el-col>
          <!-- 告警类型 -->
          <el-col :span="6">
            <el-form-item label="告警类型">
              <el-select v-model="searchForm.warnType" filterable clearable placeholder="请选择告警类型">
      ...
      <div class="table_btn" style="height: 65px; display: block;">
        <el-row>
          <el-col :span="3" v-for="(item, index) in typeOptions" :key="index">
            <div style="display: inline-block; margin: 5px 10px; font-size: 14px; cursor: pointer;padding: 10px;border-radius: 5px;" :style="{ 'border': searchForm.warnType === item.value ? '2px solid #409eff' : 'none'}" @click="typeClickFunc(item.value)">
              {{ item.label + ': ' }}<span style="color: #409EFF; font-weight: bold">{{ item.count }}</span>个
            </div>
          </el-col>
        </el-row>
      </div>
    ...
    <div class="page_body">
      <el-table ref="tableData" :data="tableData" :height="tableHeight" stripe size="small" v-loading="tableLoading" @selection-change="selectionChange">
        ...
        <el-table-column label="告警来源" prop="source" width="100" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ conversionDict(scope.row.source, sourceOptions) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="告警类型" prop="warnType" width="100" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span>{{ conversionType(scope.row.warnType) }}</span>
          </template>
        </el-table-column>
        <el-table-column label="创建人" prop="createUserName" width="120" align="center" show-overflow-tooltip />
        ...
        <el-table-column label="告警来源工单" prop="code" width="170" align="center" show-overflow-tooltip />
...
import { getByPage, dealWarning, getInfoFeedbackFiles, getTypeCount } from '@/api/xcnanr/warning'
  ...
  methods: {
    ...
    getTypes() {
      ...
      getDictList('info_warning_type').then(res => {
        const dict = res.data
        for (const d of dict) {
          this.typeOptions.push({ label: d.dictValue, value: d.dictKey, count: '-' })
        }
        this.getList()
      })
    },
    ...
    getList() {
      ...
      this.searchForm.startTime = (times && times.length>1) ? (times[0] + ' 00:00:00') : null
      this.searchForm.endTime = (times && times.length>1) ? (times[1] + ' 23:59:59') : null
      ...
      // 获取类型数量
      getTypeCount(this.searchForm).then(res => {
        for (const type of this.typeOptions) {
          type['count'] = res.data[parseInt(type['value'])] | 0
        }
      })
    },
    ...
    conversionType(code) {
      //字典回显
      let items = this.typeOptions.filter(item => String(item.value) === String(code))
      if (items != null && items.length > 0) {
