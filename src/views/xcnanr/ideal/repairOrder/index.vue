...
<!--            <el-button v-if="(scope.row.statusInfo===1) && scope.row.tijiao==='yes'&& (scope.row.createUser === userDetail.userId)" type="text" size="mini" @click="submitEvent(scope.row)"> 提 交 </el-button>-->
            ...
            <el-button v-if="([1, 2, 4].indexOf(scope.row.statusInfo) >= 0)" type="text" size="mini" @click="dispatchFunc(scope.row)" icon="el-icon-s-custom">派 单</el-button>
      ...
      <dispatch-view v-if="dialogFlag && dialogType === 'dispatch'" :dialog-flag="dialogFlag" :dialog-object="dialogObject" />
    </div>
  </basic-container>
</template>
...
import RepairOrderForm from './add.vue'
...
export default {
  name: 'RepairOrder',
  components: {xtTreeLzaySelect, getDictList, RepairOrderForm, dispatchView},
  computed: {
  ...
  methods: {
    ...
    addEvent() {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/add'
      })
    },
    /**
     * 编辑
    ...
    editEvent(row) {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/edit', query: {code: row.code, permissions: row.tijiao}
      })
    },
    /**
     * 审批
    ...
    approveEvent(row) {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/approve', query: {id: row.id, code: row.code, type: row.type, stateBiangeng: row.stateBiangeng}
      })
    },
    /**
     * 变更
    ...
    changeEvent(row) {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/change', query: {id: row.id, code: row.code, type: row.type}
      })
    },
    /**
     * 终结
    ...
    feedbackEvent(row) {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/feedback', query: {code: row.code}
      })
    },
    /**
     * 详情
    ...
    detailEvent(row) {
      this.$router.push({
        path: '/xcnanr/ideal/repairOrder/detail', query: {code: row.code}
      })
    },
    dispatchFunc(row) {
      this.dialogObject = row
      this.dialogType = 'dispatch'
