<template>
  <div>
    <el-dialog :title="getTitle(roomData)" :visible.sync="dialogVisible" width="80%" @close="handleClose"  :close-on-click-modal="false">
        ...
        <el-col :span="10">
          ...
          <div class="page_body">
            ...
            >
              ...
              <el-table-column label="类型" align="center" min-width="130">
                <template slot-scope="scope">
                  <el-select v-if="editRowIndex === scope.$index" v-model="scope.row.extAttr1" placeholder="请选择图纸类型">
                    <el-option v-for="item in dataTypes" :key="item" :label="item" :value="item"></el-option>
                  </el-select>
                  <span v-else>{{ scope.row.extAttr1 }}</span>
                </template>
              </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      editRowIndex: -1,
      dataTypes: ['机房定制图', '设备布局图', 'ups电源接线图', '光配资料图', '网络拓扑图']
    };
  },
  mounted() {
    this.getDict()
  ...
  methods: {
    ...
    updateDrawingName(row) {
      ...
      this.tableLoading = true
      editRoomDataDetail({id : row.id, name: row.name, extAttr1: row.extAttr1}).then(() => {
