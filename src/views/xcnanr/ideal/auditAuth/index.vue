  ...
  export default {
    ...
    methods: {
      ...
      detailEvent(row) {
        ...
        if (workOrderType === 1) {
          // 打开巡视工单的详情界面
          this.$router.push({ path: '/xcnanr/ideal/inspectionOrder/detail', query: { orderId: row.workId }})
        }else if (workOrderType === 2) {
          // 打开检修工单的详情界面
          this.$router.push({path: '/xcnanr/ideal/repairOrder/detail',query:{ code: row.workCode }})
        }else if (workOrderType === 3) {
          // 打开临时授权工单的详情界面
          this.$router.push({path: '/xcnanr/ideal/auditAuth/detail',query: {authCode: row.workCode}})
        }
      },
      // 设置表格高度
      setTableHeight() {
