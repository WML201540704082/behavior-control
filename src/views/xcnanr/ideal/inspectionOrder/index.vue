    ...
    <div class="page_body">
      <el-table ref="tableData" :data="tableData" :height="tableHeight" stripe size="small" v-loading="tableLoading" @selection-change="selectionChange">
        ...
        <el-table-column label="操作" prop="createTime" align="center" width="160" fixed="right" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button type="text" size="mini" v-if="scope.row.statusInfo === 1 && scope.row.createUser === userInfo.userId" @click="handleEdit(scope.row)" icon="el-icon-edit">编 辑</el-button>
<!--            <el-button type="text" size="mini" v-if="scope.row.isSubmitRole === 1 && scope.row.statusInfo === 1" @click="submitEvent(scope.row.id)">提 交</el-button>-->
            ...
            <el-button v-if="([1, 2].indexOf(scope.row.statusInfo) >= 0)" type="text" size="mini" @click="dispatchFunc(scope.row)" icon="el-icon-s-custom">派 单</el-button>
...
export default {
  components: {
    xtTreeLzaySelect, getDictList, dispatchView
  },
  computed: {
  ...
  data() {
    return {
      ...
      dialogType: null, dialogFlag: false, dialogObject: null
    }
  },
  mounted() {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push({
        path: '/xcnanr/ideal/inspectionOrder/add',
        query: {
          type: 'add',
        }
      })
    ...
    handleFeedback(row){
      this.$router.push({
        path: '/xcnanr/ideal/inspectionOrder/feedback',
        query: {
          id: row.id
        }
      })
    ...
    handleSee(row) {
      this.$router.push({
        path: '/xcnanr/ideal/inspectionOrder/detail',
        query: {
          orderId: row.id
        }
      })
