...
export default {
  ...
  methods: {
    ...
    redirectEvent(type, data) {
      ...
      // 先关闭已经打开过的tab页
      const thisTabId = document.getElementById("tab-/xcnanr/operationPlan/riskcontrol/index")
      const allTabs = thisTabId.parentElement.children
      for (let i = 0; i < allTabs.length; i++) {
        const tab = allTabs[i]
        if (tab.id.indexOf("tab-/xcnanr/RiskManagement") >= 0) {
          document.getElementById(tab.id).childNodes[1].click()
        }
      }
      // 再重新打开tab页
      this.$router.push({
        path: `/xcnanr/RiskManagement${type}`,
        query: {
          id,
          type
        }
