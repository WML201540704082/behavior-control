    ...
    <el-dialog :title="grandsonDialogTitle" :visible.sync="showChild" append-to-body width="890px" top="10vh" :show-close="true" :close-on-click-modal="false" v-el-drag-dialog>
      <el-form ref="submitForm" :model="formDetail" :rules="rules" label-suffix=":" label-width="120px" size="small" v-loading="loading">
        <el-row>
          ...
          <el-col :span="24" v-if="formDetail.platformId">
            <el-form-item label="">
          ...
          <el-col :span="12" v-if="formDetail.type === '2'">
            <el-form-item label="身份证号" :prop="formDetail.type === '2' ? 'idCard':null" >
              <el-input v-model="formDetail.idCard"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12" v-if="formDetail.type === '2'">
            <el-form-item label="外部单位" :prop="formDetail.type === '2' ? 'externalCompany' : null">
              <el-input v-model="formDetail.externalCompany" />
            </el-form-item>
          </el-col>
          <el-col :span="12">
...
export default {
  ...
  data() {
    ...
    return {
      formDetail: { name: '', idCard: '', externalCompany: '' },
      operatorType: 'add',
      pickerOptions: {
        disabledDate: (time) => {
  ...
  methods: {
    changePerson() {
      if (this.formDetail.type === '2'){
        this.$set(this.formDetail, 'name', '')
        this.$set(this.formDetail, 'idCard', '')
        this.$set(this.formDetail, 'externalCompany', '')
      }
    },
    getPersonList(){
