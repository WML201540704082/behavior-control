    ...
    <div class="page_form" style="padding-bottom: 15px">
      <el-form ref="searchForm" :model="searchForm" label-width="130px" class="xt_search_form">
        <el-row>
          ...
          <el-col :span="12" style="margin-top: 0px;" align="right">
            <!-- 查询按钮 -->
      ...
      <div class="table_btn">
        <el-button v-if="permissionList.addBtn" type="primary" style="margin-left: 5px" @click="redirectEvent('Add', null, null, null)">新增</el-button>
      ...
      <el-table ref="tableData" :data="tableData" :height="tableHeight" stripe size="small" v-loading="tableLoading" @selection-change="selectionChange">
        ...
        <el-table-column label="计划编号" prop="code" width="160" align="center" show-overflow-tooltip>
          <template slot-scope="scope">
            <span @click="redirectEvent('Detail', null, scope.row.code, null)" style="cursor: pointer; color: #59a2fd;">{{ scope.row.code }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" width="192" fixed="right" show-overflow-tooltip>
          <template slot-scope="scope">
            <el-button type="text" size="mini" v-if="permissionList.editBtn && scope.row.statusInfo === 1&& (scope.row.createUser === userDetail.userId)  " @click="redirectEvent('Edit', null, scope.row.code, null)" icon="el-icon-edit">编 辑</el-button>
            <el-button type="text" size="mini" v-if="scope.row.statusInfo === 2 && scope.row.shenhe === 'yes'" @click="redirectEvent('Approve', scope.row.id, scope.row.code,'2')" icon="el-icon-finished">审 批</el-button>
            <el-button type="text" size="mini" @click="redirectEvent('Detail', null, scope.row.code, null)" icon="el-icon-s-fold">详 情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <!-- 分页 -->
...
export default {
  ...
  methods: {
    ...
    // 跳子页面
    redirectEvent(suffixName, id, code, type) {
      let childTitle = ''
      if (suffixName === 'Add') {
        childTitle = '新增'
      } else if (suffixName === 'Edit') {
        childTitle = '编辑'
      } else if (suffixName === 'Approve') {
        childTitle = '审批'
      } else if (suffixName === 'Detail') {
        childTitle = '详情'
      }
      const path = `/xcnanr/repairPlan${ suffixName }`

      // 先关闭已经打开过的tab页
      const thisTabId = document.getElementById("tab-/xcnanr/operationPlan/repairPlan/index")
      const allTabs = thisTabId.parentElement.children
      for (let i = 0; i < allTabs.length; i++) {
        const tab = allTabs[i]
        if (tab.id.indexOf("tab-/xcnanr/repairPlan") >= 0) {
