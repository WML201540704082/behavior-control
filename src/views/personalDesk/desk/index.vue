      ...
      <div class="desk_tl">
        <div class="desk_tlt">
          ...
          <div class="desk_tabs">
            ...
            <div class="flr" v-if="activeName == 1">
              <el-input v-model="searchForm.code1" placeholder="请输入单号" size="mini"></el-input>
...
// 新增部分
import { waitForHandle, waitForHandleNum } from '@/api/personalDesk/xcnanr'
...
export default {
  ...
  methods: {
    ...
    handleView(row) {
      ...
      } else if (row.processName == '巡视计划') {
        this.$router.push({
          path: "/xcnanr/inspectPlanDetail",
          query: {
            id: row.id,
            type: 'detail'
          }
        });
      } else if (row.processName == '检修计划') {
        this.$router.push({
          path: "/xcnanr/repairPlanDetail",
          query: {
            childTitle: '详情',
            suffixName: 'Detail',
            id: row.id,
      ...
      } else if (row.processName == '检修工单') {
        this.$router.push({
          path: "/xcnanr/repairOrderDetail",
          query: {
            code: row.businessId
          }
        });
      } else if (row.processName == '维修工单') {
        debugger
        this.$router.push({
    ...
    handleAudited(row) {
      ...
      if(row.type !== '检修计划' && row.type !== '检修工单') {
    ...
    async handleOrder(item) {
      ...
      this.searchForm.code1 = undefined
    ...
    getOrderList() {
      ...
      if (this.orderType === '作业计划' || this.orderType === '作业工单') {
        ...
        todoApi({processKeys: this.processKeys}).then(res => {
          ...
          if(this.searchForm.code1)this.searchForm.orderNoList = this.searchForm.code1
