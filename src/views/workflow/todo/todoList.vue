      ...
      <div class="table-box">
        <div class="table-filter flex">
          <el-select
            ...
            :placeholder="this.$hussarT('hussar-todoList.selectProcess')"
            filterable
            clearable
          ...
          <el-input
            ...
            :placeholder="this.$hussarT('hussar-todoList.sender')"
            class="jxd_ins_elInput default"
            readonly style="width:200px;margin: 0 8px"
            clearable
            type="text"
            @focus="openUser()"/>
          <el-input
        ...
        <div
          ...
          <el-table
            ...
            <el-table-column
              ...
              align="center"/>
            <el-table-column
              :label="this.$hussarT('hussar-todoList.process')"
              prop="processName"
              align="center">
              <template slot-scope="scope">
                <el-link class="jxd_ins_elLink default newPage" style="cursor:pointer;"><span @click="showPic(scope.row)">{{ scope.row.processName }}</span></el-link>
              </template>
            </el-table-column>
            <el-table-column
              :label="this.$hussarT('hussar-todoList.node')"
              ...
              <template slot-scope="scope">
                <el-link class="jxd_ins_elLink default">
                  <span
                    ...
                    :title="getNodeName(scope.row)"
                    class="newPage"
                    @click="openRoute(scope.row)"> {{ getNodeName(scope.row) }}</span>
                  <span
                    v-else
                    :title="getNodeName(scope.row)">{{ getNodeName(scope.row) }}</span>
                </el-link>
              </template>
            </el-table-column>
            <el-table-column
              :label="this.$hussarT('hussar-todoList.sender')"
              prop="sendUserName"
              align="center"/>
            <el-table-column
              :label="this.$hussarT('hussar-todoList.creationTime')"
              prop="startTime"
              align="center"/>
            <el-table-column
              :label="this.$hussarT('hussar-todoList.taskContent')"
              prop="message"
              title="message"
              align="center">
              <template slot-scope="scope">
...
export default {
  ...
  methods: {
    ...
    queryList(isReset) {
      ...
      hussarRequest.get(TodoTaskApi.list, this.listQuery).then(res => {
        if (res.code === 10000) {
    ...
    getNodeName(row) {
      if (row.ownerName === undefined || row.ownerName === "") {
        return row.taskDefinitionName;
      }else {
        return row.taskDefinitionName + (row.ownerName === "" ? "" : "(" + row.ownerName + ")");
      }
    }
  }
}
...
.currTable ::v-deep .el-loading-mask {
  top: 34px;
}
...
.lcdp_axe.theme-config-personal-matters ::v-deep .el-tabs__item i {
  ...
  /* 流程记录——流程记录颜色 */
  .lcdp_axe.theme-config-personal-matters .list-box .table-box ::v-deep .flowfile.workflow .el-tabs .el-tabs__content #pane-second .el-table .el-table__body-wrapper .el-table__body tbody tr td:nth-child(6) .cell div {
    color: var(--theme-color) !important;
  }
