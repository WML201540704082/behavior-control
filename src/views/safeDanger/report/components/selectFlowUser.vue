...
<script>
import { nextWorkItem } from "@/api/safeDanger/workorder/bpm/bpm";
import { queryNextFlowUsers } from "@/api/safeDanger/report"
export default {
  name: "selectFlowUser",
  props: {
    idd: {
  ...
  methods:{
        ...
        if(this.treeClickCount === 1) {
          ...
          if(data.itemType === '03' || data.text === '结束') {
            if(this.tableData.length === 0) {
              ...
              if ( this.formData.status != 1 && this.formData.status != 4 ) {
                this.$message.warning('流程节点只能选择一位审批人！');
    ...
    initTreeVue:function() {
      ...
      let data;
      if(this.rectificationStatus === 0){ // 通过
        data = {
        ...
        data = {
          processInstId: this.processInstID,
          passStatus: 1,
          returnOrFinishStatus: this.rectificationStatus-1,
          id: this.idd
        }
      }
    ...
    addTableData:function() {
      ...
      if ( dataTree.length > 1 && this.formData.status != 1 && this.formData.status != 4 ) {
        _this.$message.warning('流程节点只能选择一位审批人！');
        return;
      }
