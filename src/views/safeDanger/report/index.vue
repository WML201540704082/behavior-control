<template>
  <div>
        ...
        <div class="page_form">
          <el-form class="xt_search_form" ref="queryForm" :model="queryParams" label-width="105px">
            <el-row>
              ...
              <el-col :span="6">
                <el-form-item label="隐患一级分类:" prop="hiddenCategoryName">
                  <el-select v-model="queryParams.hiddenCategoryName" clearable size="mini" placeholder="">
                    <el-option
              ...
              <el-col :span="6">
                <el-form-item label="整改方式:" prop="rectificationMethod">
                  <el-select v-model="queryParams.rectificationMethod" size="mini" clearable placeholder="">
                    <el-option
              ...
              <el-col :span="6">
                <el-form-item label="年度查询:" prop="yearQuery">
                  <el-select v-model="queryParams.yearQuery" size="mini" clearable placeholder="">
            ...
            <!-- <div> -->
              <el-row class="btnLine">
                ...
                <el-button type="primary" size="mini" style="margin-left: 5px;" icon="el-icon-delete"
                           v-hasPermi="['hidden:order:remove']"
                ...
                <el-button type="primary" size="mini" style="margin-left: 5px;" icon="el-icon-edit"
                           v-hasPermi="['hidden:batch:upload']"
                           :disabled="!(multipleSelection.length == 1 && multipleSelection[0] && multipleSelection[0].releaseStatus != 1)"
                           @click="handleUpdate2">导入修改
                </el-button>
                <el-button type="primary" size="mini" style="margin-left: 5px;" icon="el-icon-delete"
                           v-hasPermi="['hidden:province:del']"
                           :disabled="!(multipleSelection.length > 0 && multipleSelection.every(el=> el.releaseStatus != 1) )"
                ...
                <el-button type="primary" size="mini" style="margin-left: 5px;" icon="el-icon-plus"
                  ...
                  <!-- multipleSelection[0].releaseStatus != 1 -->
                </el-button>
                <!-- v-hasPermi="['hidden:batch:release']" -->
                <el-button type="primary" size="mini" style="margin-left: 5px;" icon="el-icon-refresh"
                           v-hasPermi="['hidden:order:recover']"
                           :disabled="recoverDis"
                           @click="handleOrderRecover">追回
                </el-button>
              </el-row>
            <!-- </div> -->
          </el-form>
          ...
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="{row}">
                <span> {{ row.hiddenCategoryName }} </span>
              </template>
            </el-table-column>
            <el-table-column
              prop="hiddenNumber"
    ...
    <el-dialog :modal-append-to-body="false" :title="dialogTitle" style="padding: 0px;"
      ...
      <Detail v-if="dialogVisible" @closeRoom="closeDialog" :detailId="detailId" :is-edit="isEdit" :is-info="isInfo"/>
    </el-dialog>


    <!-- 工作日志 -->
    <el-dialog :modal-append-to-body="false" title="工作日志" :visible.sync="worklogVisible" width="80%">
      <WorkLog v-if="worklogVisible" @closeRoom="worklogVisible = false" :detailId="detailId" :workflowId="workflowId"/>
    </el-dialog>
    ...
    <el-dialog title="批量导入清单展示" :modal-append-to-body="false" :close-on-click-modal="false" style="padding: 0px;" width="80%"
      ...
      <ImportTable v-if="importTableVisible" @closeRoom="closeDialog" :importTable="importTable"/>
    </el-dialog>
  </div>
</template>
...
import {orderList, orderRemove, orderRecover, month} from "@/api/safeDanger/report"
...
export default {
  ...
  computed: {
    ...
    recoverDis() {
      // 当前工单带99（市级） 不带99（县级）
      return !(this.multipleSelection.length == 1 && this.multipleSelection[0] && ((this.multipleSelection[0].secondCompany.slice(-2) == '99' && this.multipleSelection[0].status == 1) || (this.multipleSelection[0].secondCompany.slice(-2) != '99' && this.multipleSelection[0].status == 8)))
    }
  },
  components: {
    Detail,
  ...
  methods: {
    handleOrderRecover() {
      orderRecover({id: this.multipleSelection[0].id}).then(res => {
        if (res.code == 200) {
          this.getList();
    ...
    handleUpdate() {
      ...
      this.isEdit = true
      this.isInfo = false
      // if (this.multipleSelection[0].version == "1.0.0") { //老版本
      //   this.detailId = this.multipleSelection[0].id;
      //   this.dialogTitle = '编辑';
      //   this.dialogVisibleOld = true;
      //   this.isEdit = true
      //   this.isInfo = false
      // } else { //新版本
      //   this.detailId = this.multipleSelection[0].id;
    ...
    exportExcel() {
      this.$confirm('全量数据导出较慢，请耐心等待。', '警告', {
        ...
        this.download('/msdp-hiddendanger/order/export', {
          ...this.queryParams
