  ...
  <basic-container>
    ...
    <div class="page_body">
      ...
      >
        ...
        <el-table-column prop="userType" label="用户类型" min-width="90" align="center" show-overflow-tooltip>
          <template  slot-scope="scope">
            <span>{{scope.row.type == 0 ? '管理员' : '普通用户'}}</span>
          </template>
        </el-table-column>
        <el-table-column label="可登录时间段" min-width="250" align="center" show-overflow-tooltip>
          <template  slot-scope="scope">
            <span>{{scope.row.loginBeginTime + '~' + scope.row.loginEndTime}}</span>
          </template>
        </el-table-column>
